...
void Game::Setup(void)
{
    ...
    // Keep track of collisions - (DANIEL GUO)
    collisions = 0;
    // Load textures
    SetAllTextures();
    ...
    // Setup other objects
    game_objects_.push_back(new GameObject(glm::vec3(-1.0f, 1.0f, 0.0f), sprite_, &sprite_shader_, tex_[1]));
    game_objects_.push_back(new GameObject(glm::vec3(1.0f, -0.5f, 0.0f), sprite_, &sprite_shader_, tex_[2]));
    // New objects - (DANIEL GUO)
    game_objects_.push_back(new GameObject(glm::vec3(2.0f, 2.0f, 0.0f), sprite_, &sprite_shader_, tex_[5]));
    game_objects_.push_back(new GameObject(glm::vec3(1.0f, 3.0f, 0.0f), sprite_, &sprite_shader_, tex_[6]));
    game_objects_.push_back(new GameObject(glm::vec3(1.0f, 2.0f, 0.0f), sprite_, &sprite_shader_, tex_[6]));
    game_objects_.push_back(new GameObject(glm::vec3(1.0f, 1.0f, 0.0f), sprite_, &sprite_shader_, tex_[6]));
...
void Game::Update(glm::mat4 view_matrix, double delta_time)
{
    ...
    for (int i = 0; i < game_objects_.size(); i++) {
        ...
        for (int j = i + 1; j < (game_objects_.size()-1); j++) {
            ...
            if (distance < 0.8f) {
               glm::vec3 tmpPos = other_game_object->GetPosition();
               game_objects_.erase(game_objects_.begin()+j);
               //game_objects_.insert(game_objects_.begin() + j,new GameObject(tmpPos, sprite_, &sprite_shader_, tex_[7]));
               tmp_game_objects_.push_back(new GameObject(tmpPos, sprite_, &sprite_shader_, tex_[7]));
               //increase collisions by 1
               collisions++;
               if (collisions == 3) {
                   //player is erased and an explosion is shown
                   tmp_game_objects_.push_back(new GameObject(game_objects_[0]->GetPosition(), sprite_, &sprite_shader_, tex_[7]));
                   game_objects_.erase(game_objects_.begin());
               }
            }
            // Render game object(permenant objects)
            current_game_object->Render(view_matrix, current_time_);
        }

        //delete all explosion sprites every few seconds - (DANIEL GUO)
        for (int i = 0; i < tmp_game_objects_.size(); i++) {
            // Get the current game object
            GameObject* current_game_object = tmp_game_objects_[i];

            // Update the current game object
            current_game_object->Update(delta_time);
            //deletes most(all) objects every 5 seconds
            if ((int)glfwGetTime() % 5 == 0) {
                tmp_game_objects_.erase(tmp_game_objects_.begin());
                //if collisions was 3 when the last explosions were removed, show game over.
                if (collisions == 3) {
                    glfwDestroyWindow(window_);
                    std::cout << "GAME OVER\n";
                }
            }
            // Render game object(temporary objects)
            current_game_object->Render(view_matrix, current_time_);
        }
        // Render game object(background object)
        current_game_object->Render(view_matrix, current_time_);
    }
}
...
void Game::Controls(double delta_time)
{
    ...
    // Sped up by (DANIEL GUO)
    float speed = 1;
    float motion_increment = speed*delta_time;
    //disables controls if collisions >= 3 - (DANIEL GUO)
    if (collisions < 3){
